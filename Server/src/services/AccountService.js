...
AccountService.Login = async (ctx,Tra,UserName,Password) => {
    try {
        ...
        if (await ctx.TokenCache.select(UserObject.id)){
            await ctx.TokenCache.replace(UserObject.id,{ AccessToken:AccessToken},config.secret.TokenValidPeriod)
        }
        else {
            await ctx.TokenCache.add(UserObject.id,{ AccessToken:AccessToken },config.secret.TokenValidPeriod)
        }
        return AccessToken
    }catch (e) {
        throw e
